function [eout,thresh,gv_45,gh_135] = edgeGradient(varargin)

%This code has been modified by Vallotton to return the magnitude of the
%projected gradient from an interior droplet point, declared as a global
%variable. Our apologies to the purists.
%the modifications concentrate in lines 263 and after.
...
%   $Revision: 5.26.4.7 $  $Date: 2008/04/03 03:10:52 $
...
if strcmp(method,'canny')
    ...
    %This is where modifications were made to the original edge.m code for
    %returning a projected gradient magnitude
    xp23=meshgrid(1:size(aSmooth,2),1:size(aSmooth,1));
    yp23=meshgrid(1:size(aSmooth,1),1:size(aSmooth,2))';
    ...
    normfact23=(xp23.^2+yp23.^2).^0.5;
    xp23=xp23./normfact23;
    yp23=yp23./normfact23;
    projx23=ax.*xp23;
    projy23=ay.*yp23;
    
    %Subtlety. you know that the gradient you want generally
    %corresponds to a dark to bright transition but you don't want to kill
    %the information completely when the droplet boundary is characterized
    %by an inverse transition due to illumination issues.
    projx23(projx23<-2)=-2;
    projy23(projy23<-2)=-2;
   
    mag=(projx23.*projx23+projy23.*projy23).^0.5;%    
    magmax = max(mag(:));
    if magmax>0
        mag = mag / magmax;   % normalize
    end
    eout=mag;
    return; %the projected gradient magnitude is returned. it's all we need...
    ...
    if isempty(thresh)
        ...
        thresh = [lowThresh highThresh];closeall
    ...
    if ~isempty(idxStrong) % result is all zeros if idxStrong is empty
        ...
        e = bwmorph(e, 'thin', 1);  % Thin double (or triple) pixel wide contours%figure;imshow(e,[])
    end
